...
class Buytolet extends CI_Controller
{
	...
	public function insertToSelzyDashboard($fname, $lname, $email, $phone)
	{
		...
		$method = 'https://api.selzy.com/en/api/subscribe?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy&list_ids=323&fields[email]=' . $email . '&fields[Name]=' . $fname . '&fields[Surname]=' . $lname . '&fields[phone]=' . $phone . '&double_optin=3&overwrite=0';
	...
	{
		// Partnero script content to track users on signup, its required ID which is not set in the regJS
		$partneroScript = "
        <script>
            po('customers', 'signup', {
                data: {
	...
	public function scheduleInsp()
	{
		...
		if ($res) {
			...
			// Send Unione Template email for inspection request to CEO
	...
	public function outright_offer_letter($ref = 'aeorjf787g90dfdfdfdf', $email = 'seuncrowther@yahoo.com', $phone = '080********', $property_name = 'Olivia Court 5 Bedroom Duplex', $buyer_name = 'Pidah Tnadah', $property_address = '1B Admiralty way, Lekki Phase 1', $property_city = 'Lekki', $property_state = 'Lagos', $total_amount = '5000000', $trans_fee = '400000', $bed = '5', $apt_type = '5 Bedroom')
	{
		...
		//<div style="width:100%;height:800px;padding:20px;position:relative"><img src="https://sbuy.smallsmall.com/assets/images/cs-bg-1.png" width="100%" /><div style="width:70%;height:100px;position:absolute;top:100px;left:10%;background:#333;z-index:99999999999"></div></div>
	...
	public function filter_properties()
	{
		...
		// Filter values from POST data
		{
			...
			$data['states'] = $this->buytolet_model->get_states(); // Added State
	...
	public function send_payment_email($subject, $email, $property_name, $shares_amount, $name, $amount)
	{
		...
		$method = 'https://api.selzy.com/en/api/sendEmail?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy&email=' . $email . '&sender_name=Buy+Small+Small&sender_email=hello@buysmallsmall.ng&subject=Sample+Subject&body=' . $body . '&list_id=323';
	...
	public function self_shares_email($subject, $email, $shares_amount, $property_name, $amount, $completion_date, $buyback_rate, $hold_period, $migration_date, $name)
	{
		...
		$method = 'https://api.selzy.com/en/api/sendEmail?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy&email=' . $email . '&sender_name=Buy+Small+Small&sender_email=hello@buysmallsmall.ng&subject=' . $subject . '&body=' . $body . '&list_id=323';
	...
	public function send_personalized_email($subject, $email, $email_type, $link, $name = '')
	{
		...
		if ($email_type == 'verification') {
			...
			$method = 'https://api.selzy.com/en/api/subscribe?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy&list_ids=323&fields[email]=' . $email . '&fields[Name]=' . $name . '&double_optin=3&overwrite=0';
	...
	public function test_email()
	{
		...
		$verify = 'https://api.selzy.com/en/api/sendEmail?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy&email=seuncrowther@gmail.com&sender_name=Seun&sender_email=hello@buysmallsmall.ng&subject=Sample+Subject&body=' . $body . '&list_id=323';

		$subscribe = 'https://api.selzy.com/en/api/subscribe?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy&list_ids=323&fields[email]=seuncrowther@gmail.com&fields[Name]=Oluwaseun+Crowther&double_optin=0&overwrite=0';

		$lists = 'https://api.selzy.com/en/api/getLists?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy';

		$get_contact = 'https://api.selzy.com/en/api/getContact?format=json&api_key=6u7xpfqygdmqxpkz1uonenrxwpn43z3bhibuzccy&email=seuncrowther@gmail.com';
